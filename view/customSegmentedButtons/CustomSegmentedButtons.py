...
class CustomSegmentedButtons(ctk.CTkFrame):
    def __init__(self, root: any, label: str, textValues: List[str], imgValues: List[ctk.CTkImage]):
        super().__init__(root)
        ...
        for btnId in range(len(textValues)):
            btn = ctk.CTkButton(self, text=textValues[btnId], image=imgValues[btnId], compound="top", width=100, fg_color=PRESSED_BTN_COLOR, command=lambda btnId=btnId: self.btnClick(btnId))
            
            padx = [PADDING_S if btnId == 0 else PADDING_XS, PADDING_S if btnId == len(textValues)-1 else PADDING_XS]
